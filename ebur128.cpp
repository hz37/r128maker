#pragma warn -8004  // Added Hens Zimmerman to avoid
                	// 'errcode is assigned a value that is never used'
...
double ebur128_gated_loudness(ebur128_state** sts, size_t size,
  ...
  for (i = 0; i < size; i++) {
    if (sts[i] && (sts[i]->mode & EBUR128_MODE_I) != EBUR128_MODE_I) {
      return 0.0; // EDITED HZ; was div by zero / 0.0;
    }
  }
  ...
  if (!above_thresh_counter) return -1.0; // EDITED HZ; was div by zero / 0.0;
